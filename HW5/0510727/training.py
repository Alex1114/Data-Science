...
def test_collate(sample):
    graphs, feats =map(list, zip(*sample))
    graph = dgl.batch(graphs)
    feats = torch.from_numpy(np.concatenate(feats))
    return graph, feats

def evaluate(feats, model, subgraph, epoch, loss_data):
    with torch.no_grad():
        model.eval()
        model.g = subgraph
        i = 0 
        ...
        fp = open("./result/result_epoch{}_loss{:.6f}.csv".format(epoch,loss_data), "a")
...
def main(args):
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size, collate_fn=test_collate)
    ...
    for epoch in range(args.epochs):
        ...
        if loss_data < best_loss:
            ...
            for batch, test_data in enumerate(test_dataloader):
                subgraph, feats = test_data
                feats = feats.to(device)
                evaluate(feats.float(), model, subgraph, epoch, loss_data)

            
        # early stop
        #torch.save(model.state_dict(), "./models/{}_epoch{}_loss{:.5f}.pt".format('gat', epoch+1, loss_data))
        # if epoch % 5 == 0:
        #     score_list = []
        #     val_loss_list = []
