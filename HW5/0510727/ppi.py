...
class PPIDataset(object):
    ...
    def _load(self):
        ...
        print('Loading G...')
        if self.mode == 'train':
            with open('./hw5_dataset/train_graph.json'.format(dir)) as jsonfile:
                g_data = json.load(jsonfile)
            self.labels = np.load('./hw5_dataset/train_labels.npy')
            self.features = np.load('./hw5_dataset/train_feats.npy')
            self.graph = DGLGraph(nx.DiGraph(json_graph.node_link_graph(g_data)))
            self.graph_id = np.load('./hw5_dataset/train_graph_id.npy')
        # if self.mode == 'valid':
        ...
        if self.mode == 'test':
            with open('./hw5_dataset/test_graph.json'.format(dir)) as jsonfile:
                g_data = json.load(jsonfile)
            # self.labels = np.load('{}/ppi/test_labels.npy'.format(dir))
            self.features = np.load('./hw5_dataset/test_feats.npy'.format(dir))
            self.graph = DGLGraph(nx.DiGraph(json_graph.node_link_graph(g_data)))
            self.graph_id = np.load('./hw5_dataset/test_graph_id.npy'.format(dir))
    ...
    def _preprocess(self):
        ...
        if self.mode == 'test':
            ...
            self.test_graphs = []
            # self.test_labels = []
            for test_graph_id in range(21, 25):
...
class LegacyPPIDataset(PPIDataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'test':
            return self.test_graphs[item], self.features[self.test_mask_list[item]]